...
1. **SQLite移行**: library.json → SQLite（クライアント + サーバ）
   - メタデータ追加の土台。部分更新が効くようになる
   - スキーマバージョン管理 + マイグレーション機構
2. **1バイナリ統合**: TrussPhotoServer を TrussPhoto に統合
   - --server / --remote モード実装
   - tpDataPath / tpCachePath のプラットフォーム別パス
3. **リッチメタデータ構造**: PhotoEntryの拡張（memo, annotations, tags）
   - フィールド別 updatedAt による同期
   - UIでのメモ編集（シングルビュー）
4. **スマートプレビュー生成**: libjxl 導入、サーバ側で JPEG XL 生成
5. **XMP読み書き**: exiv2でdc:description/dc:subjectの読み書き、サイドカー生成
6. **embedding基盤**: tcxOnnxアドオン、サーバにCLIP推論、embeddingテーブル
7. **ベクトル検索**: sqlite-vss導入、類似検索API
8. **MCP拡張**: 検索・メモ追加・関連写真取得のツール群
