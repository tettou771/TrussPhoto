#!/usr/bin/env python3
"""
Export LINE CLIP Japanese Base v2 to ONNX (vision + text).

Model: line-corporation/clip-japanese-base-v2
  - Vision: EVA02-B/16 (timm, 224px) -> 256-dim
  - Text:   rinna/japanese-roberta-base (SentencePiece) -> 256-dim
...
Output (in ~/Library/Application Support/TrussPhoto/models/):
  line-clip-ja-v2-vision.onnx   - Vision encoder
  line-clip-ja-v2-text.onnx     - Text encoder
  line-clip-ja-v2-spiece.model  - SentencePiece tokenizer model
"""
...
def find_spiece_model(tokenizer):
    ...
    # Try wrapped tokenizer (CLYPTokenizer wraps T5Tokenizer)
    if hasattr(tokenizer, 'tokenizer') and hasattr(tokenizer.tokenizer, 'vocab_file'):
        if os.path.exists(tokenizer.tokenizer.vocab_file):
            return tokenizer.tokenizer.vocab_file
    ...
    # Search in tokenizer's saved dir
...
def main():
    ...
    vision_onnx_path = os.path.join(models_dir, "line-clip-ja-v2-vision.onnx")
    text_onnx_path = os.path.join(models_dir, "line-clip-ja-v2-text.onnx")
    spiece_path = os.path.join(models_dir, "line-clip-ja-v2-spiece.model")
    ...
    # Patch: CLYPModel lacks all_tied_weights_keys needed by transformers >=5.x
    from transformers import AutoConfig
    config = AutoConfig.from_pretrained(MODEL_PATH, trust_remote_code=True)
    model_cls = type(AutoModel.from_config(config, trust_remote_code=True))
    ...
    # --- Check dimensions ---
    dummy_image = processor(images=Image.new("RGB", (224, 224)), return_tensors="pt")
    # CLYPTokenizer handles tensor conversion internally, don't pass return_tensors
    dummy_text = tokenizer(["test"], padding="max_length",
                           max_length=MAX_SEQ_LEN, truncation=True)
    with torch.no_grad():
        img_feat = model.get_image_features(**dummy_image)
        txt_feat = model.get_text_features(**dummy_text)
    ...
    # Merge external data into single ONNX file
    ext_data = vision_onnx_path + ".data"
    if os.path.exists(ext_data):
        print("  Merging external weights into ONNX file...")
        model_onnx = onnx.load(vision_onnx_path, load_external_data=True)
        onnx.save_model(model_onnx, vision_onnx_path,
    ...
    dummy_pos = torch.arange(MAX_SEQ_LEN, dtype=torch.long).unsqueeze(0)

    # Check if model accepts position_ids
    has_position_ids = "position_ids" in dummy_text
    print(f"  Model uses position_ids: {has_position_ids}")

    with torch.no_grad():
        test_out = text_wrapper(dummy_ids, dummy_mask, dummy_pos)
    print(f"  Test output shape: {test_out.shape}, norm: {torch.norm(test_out, dim=1).item():.4f}")

    torch.onnx.export(
        text_wrapper,
        (dummy_ids, dummy_mask, dummy_pos),
        text_onnx_path,
    ...
    # Merge external data into single ONNX file
    ext_data = text_onnx_path + ".data"
    if os.path.exists(ext_data):
        print("  Merging external weights into ONNX file...")
        model_onnx = onnx.load(text_onnx_path, load_external_data=True)
        onnx.save_model(model_onnx, text_onnx_path,
                        save_as_external_data=False)
        os.remove(ext_data)
    ...
    print(f"Copying SentencePiece model...")
    src = find_spiece_model(tokenizer)
    if src is None:
        print("  WARNING: Could not find spiece.model file")
    ...
    with torch.no_grad():
        pt_img_feat = model.get_image_features(**image_inputs)
    ...
    for text in test_texts:
        text_inputs = tokenizer([text], padding="max_length",
                                max_length=MAX_SEQ_LEN, truncation=True)
        with torch.no_grad():
            pt_txt_feat = model.get_text_features(**text_inputs)
    ...
    for text in test_texts:
        ...
        ort_txt = text_session.run(["text_embeds"], ort_feed)[0][0]
    ...
    print(f"  Text inputs: input_ids + attention_mask + position_ids (3 inputs)")
